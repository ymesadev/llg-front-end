...
const eligibilityQuestions = [
  ...
  { id: "visited", question: "Did you visit or interact with Kin Insurance's website (Kin.com) within the last 2 years?", required: true },
  { id: "florida", question: "Were you located in Florida at the time you visited or interacted with Kin Insurance's website?", required: true },
  { id: "submitted", question: "Did you request a quote, create an account, or submit personal information through Kin Insurance's website?", required: true }
];
...
export default function QualifyPage() {
  ...
  useEffect(() => {
    trackEvent("qualify_page_view", { case_type: "kin" });
  }, []);
  ...
  const goToNextStep = () => {
    ...
    if (hasNo) {
      trackEvent("qualify_disqualified", { case_type: "kin", step: currentStep });
    ...
    trackEvent("qualify_step_complete", { case_type: "kin", from_step: currentStep });
    setDirection("next");
    setIsAnimating(true);
  ...
  const handleSubmit = (e) => {
    ...
    localStorage.setItem('kin-contact', JSON.stringify({ email: contactInfo.usedEmail, name: contactInfo.name, phone: contactInfo.phone }));
        trackEvent("qualify_contact_submitted", { case_type: "kin" });
      ...
      <div className={styles.container}>
        <div className={styles.content}>
          <div className={styles.disqualified}>
            ...
            <button className={styles.primaryButton} onClick={() => router.push("/kin-insurance-privacy-torts")} >Return to Homepage</button>
          </div>
        </div>
      </div>
    );
